...
# A failure HERE is setup, not execution: under set -e it would otherwise
# replace an already-recorded deployment failure with mktemp's own status.
archive=$(mktemp) || { echo "ctl-ci: cannot create the artifact archive file" >&2; [ "$rc" -eq 0 ] || exit "$rc"; exit 2; }
