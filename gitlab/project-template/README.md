...
3. Replace `192.0.2.10` in `inventory/production/hosts` with your target and select its
...
Keep the environment map pointed at the directory `inventory/production`, and keep
the host file inside it extensionless. A directory source skips files matching
`inventory_ignore_extensions`, and that list has changed between Ansible releases
and can be overridden by a site `ansible.cfg`; a skipped file yields an empty group,
and a play with no hosts succeeds, so the deployment would report green having
touched nothing. `playbooks/site.yml` asserts the group is non-empty for that
...
Validation uses `inventory/validation/hosts.ini` — an explicit file, so its suffix
is safe — in a separate directory with no adjacent `group_vars`, and does not execute the play. Keep production
