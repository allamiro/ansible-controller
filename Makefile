...
# Use the container's ANSIBLE_CONFIG, including explicit site overrides.
# XDG_CACHE_HOME: the playbooks mount is read-only; keep caches writable.
lint:
	docker exec -i -e XDG_CACHE_HOME=/tmp/.cache ansible-controller sh -c 'cd /configs/playbooks && ansible-lint'
