...
# the receptor node id being authorised, and no DNS name outside the allowlist.
# A CSR smuggling a second identity or an ingress hostname (the impersonation
# paths mTLS exists to block) is refused before signing.
#
...
[ -f "$MESH_SECRETS/ca/ca.key" ] || die "no CA key at $MESH_SECRETS/ca — restore the offline key to sign"

assert_receptor_id "$CSR" req "$ID" "$@"
rimg CSR="$CSR" ID="$ID" NOT_AFTER="$CERT_NOT_AFTER" -- '
  mkdir -p "/pki/issued/$ID"
  receptor --cert-signreq req="/pki/$CSR" cacert=/pki/ca/ca.crt cakey=/pki/ca/ca.key \
    outcert="/pki/issued/$ID/tls.crt" verify=yes notafter="$NOT_AFTER"
