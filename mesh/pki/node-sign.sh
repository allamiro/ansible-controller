...
#   mesh/pki/node-sign.sh <csr-path-relative-to-MESH_SECRETS> <node-id>
. "$(dirname "$0")/common.sh"

CSR="${1:?usage: node-sign.sh <csr-rel-path> <node-id>}"
ID="${2:?usage: node-sign.sh <csr-rel-path> <node-id>}"
