...
| `mesh-status` exits nonzero: "no ingress responded" | Both ingresses are down — the control plane itself, not a node, is the problem | `docker compose ps` and `docker logs receptor-controller` / `receptor-controller-b` on the control host; `make mesh-up` restarts them. Dispatches during the outage were refused pre-submit ("no live receptor control socket") — nothing was executed, re-run them |
