...
| A job is recorded `submit-ambiguous` | The submission left the orchestrator without returning a unit id, so nothing may assume it did or did not run. Its slot stays reserved by a `.hold`, `--collect` has no unit to re-attach to, and the GitLab integration's `ctl-run` refuses further mesh dispatches while any record is non-final | Correlate the record with both ingress work lists and record the verdict yourself: follow [Resolve an ambiguous submission](RUNBOOK.md#resolve-an-ambiguous-submission). Do not clear the slot `.hold` or finalize the record on absence from one work list alone |
